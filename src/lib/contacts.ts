...
//   email: string;
// };
// type FormEachCardType = {
//   id: string;
//   contact: EachCardType;
//   afterSave?: () => void;
// };
...
export function useContacts() {
  const [contacts, setContacts] = useGlobalState("contacts");
  ...
  async function updateContact(
    ...
    const newContacts = contacts.map((c) => {
      if (c.id === id) {
        ...
        return {
          id,
          name: attrs.name?.toString() || c.name,
          role: attrs.role?.toString() || c.role,
          email: attrs.email?.toString() || c.email,
        };
      } else {
        return c;
      }
